...
export class ProjectsTranslateService {
  ...
  private dict: Record<string, Record<string, string>> = {
    es: {
      ...
      'projects.lexia_colombia': '    Asistente legal con inteligencia artificial especializado en legislación colombiana. Implementa arquitectura RAG para consultar artículos de la ley, búsqueda semántica mediante embeddings y generación de respuestas con citas verificables utilizando múltiples proveedores de IA.',
      ...
      'projects.monetary_control': 'Aplicación para llevar el control monetario de gastos, ganancias y pérdidas. Permite gestionar todo lo financiero con registro de transacciones, cálculo de balances, reportes de ingresos y egresos, y seguimiento del estado financiero personal.',
      'projects.control_tiempo': 'Aplicación para llevar un orden de las tareas que se asignan a uno. Permite gestionar, organizar y dar seguimiento a las tareas asignadas con sistema de prioridades, estados y fechas de entrega.',
    ...
    en: {
      ...
      'projects.lexia_colombia': 'Legal assistant with artificial intelligence specialized in Colombian legislation. Implements RAG architecture to consult law articles, semantic search through embeddings, and response generation with verifiable citations using multiple AI providers.',
      ...
      'projects.sigr': 'Integrated Restaurant Management System - Web application for comprehensive restaurant management, allowing the handling of orders, reservations, menu, users and cash operations from a single platform. Includes authentication with role-based access control, dashboard with management indicators and administrative modules to optimize daily business operations.',
      'projects.world_pizza': 'Responsive restaurant-style website, modern and fast.',
      'projects.spotify': 'Music streaming application similar to Spotify with JWT authentication and local audio playback. Allows users to log in and play songs stored on the server.',
      ...
      'projects.monetary_control': 'Application for monetary control of expenses, gains and losses. Allows managing everything financially with transaction recording, balance calculation, income and expense reports, and personal financial status tracking.',
      'projects.control_tiempo': 'Application for keeping track of assigned tasks. Allows managing, organizing and following up on assigned tasks with priority system, status and due dates.',
